...
/**
 * End-to-end tests for the level catalog endpoint (v2).
 *
...
describe('Levels endpoints (e2e)', () => {
  ...
  describe('GET /api/levels', () => {
    ...
    it('should_return_a_published_level_with_its_full_v2_structure', async () => {
      ...
      expect(level.board.version).toBe(2);
      expect(level.board.rows).toBe(3);
      expect(level.board.cols).toBe(3);
      expect(level.board.arrows).toHaveLength(1);
      expect(level.board.walls).toEqual([]);
      expect(level.board.collectibles).toEqual([]);
    });
    ...
    it('should_preserve_the_arrow_direction_through_the_http_response', async () => {
      await levels.save(buildLevel({ published: true }));
      const response = await request(app.getHttpServer()).get('/api/levels');
      const arrow = response.body[0].board.arrows[0];
